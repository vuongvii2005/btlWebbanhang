...
2. Tạo một database mới tên **`vy_food`** (charset `utf8mb4_unicode_ci`)
3. Chọn database vừa tạo → tab **Import** → chọn file `database/data_hien_tai.sql` → nhấn **Go** để import cấu trúc bảng và dữ liệu mẫu
4. Sau đó import tiếp các file trong `database/migrations/` (theo đúng thứ tự tên file) nếu có, để cập nhật các thay đổi mới nhất:
